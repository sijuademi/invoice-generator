...
// ── Label ─────────────────────────────────────────────────────────────────────
export function FieldLabel({ className, error, children, rightSlot, ...props }) {
  return (
    <LabelPrimitive.Root
      className={cn(
        "flex items-center justify-between",
        "text-xs font-medium tracking-tight mb-[6px]",
        error ? "text-brand-red" : "text-label",
        className
      )}
      {...props}
    >
...
export const Input = forwardRef(function Input(
  ...
  return (
    <input
      ref={ref}
      className={cn(
        "w-full bg-surface-alt border rounded-[4px]",
