...
const TrashIcon = () => (
	<svg width="13" height="16" viewBox="0 0 13 16" aria-hidden>
		<path
			...
			className="fill-body group-hover:fill-brand-red transition-colors text-light-label"
		/>
	</svg>
);
	...
	return (
		<section>
			...
			{fields.length > 0 && (
				<div className="hidden sm:grid grid-cols-[1fr_64px_120px_80px_32px] gap-3 mb-2 ">
					{["Item Name", "Qty.", "Price", "Total", ""].map((h, i) => (
						<span
							key={i}
							className="text-xs text-label font-medium text-light-label"
						>
							{h}
						</span>
					))}
			...
			<div className="flex flex-col gap-4 sm:gap-3">
				{fields.map((field, idx) => {
					...
					return (
						<div
							key={field.id}
							className="grid grid-cols-[1fr_32px] sm:grid-cols-[1fr_64px_120px_80px_32px] gap-3 items-end"
						>
							{/* Name */}
							<div className="col-span-3 sm:col-span-1">
								<span className="sm:hidden text-xs text-light-label font-medium block mb-1.5">
									Item Name
								</span>
								<Input
							...
							<div className="flex gap-4">
								...
								<div className="sm:hidden col-span-3 grid grid-cols-2 gap-3">
									...
									<div className="flex flex-col justify-end pb-1">
										<span className="text-xs text-light-label font-medium mb-1.5">
											Total
										</span>
										<span className="text-xs font-bold text-light-label py-3">
											£{lineTotal(idx)}
										</span>
									</div>
								</div>
							...
							{/* Total */}
							<div className="hidden sm:flex items-center text-xs font-bold text-light-label pb-3.5">
								£{lineTotal(idx)}
							</div>
							...
							<button
								...
								onClick={() => remove(idx)}
								className="group flex items-end pb-3.5 justify-center cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-red rounded"
							>
								<Trash className="text-light-label" />
							</button>
						</div>
					);
				})}
			...
			<Button
				...
				className="w-full mt-12 sm:mt-4.5"
			>
				+ Add New Item
			</Button>
